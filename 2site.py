...
APARENTEMENTE, NO ANDA PORQUE EL KERNEL NO ES UN OPERADOR LINEAL EN kex
"""
...
ppm, spectrum_2 = simulate_two_site_exchange(
    k_ex=10,                       # intercambio en Hz
)

simulated_ppm = ppm
simulated_experiment = (spectrum + spectrum_2)
simulated_exp_max = np.max(simulated_experiment)
# simulated_experiment /= simulated_exp_max
# spectrum /= simulated_exp_max
# spectrum_2 /= simulated_exp_max

# Add Gaussian noise with SNR = 50
signal_power = np.mean(simulated_experiment ** 2)
noise_power = signal_power / 50**2
noise = np.random.normal(0, np.sqrt(noise_power), simulated_experiment.shape)
simulated_experiment += noise


plt.figure(figsize=(8, 4))
plt.plot(simulated_ppm, simulated_experiment, '-')
plt.plot(ppm, spectrum_2, linestyle='--', color='orange', label='k_ex=1 Hz')
plt.plot(ppm, spectrum, linestyle='--', color='green', label='k_ex=100 Hz')

plt.plot(ppm,  (spectrum + spectrum_2), linestyle='--', color='red', label='k_ex=100 Hz')
...
def KERNEL(ppm, k_ex):
    ...
    nu_L=116.6e6,                  # Larmor frequency in Hz
    amp = 10.0,                # amplitude of the lineshape
    )
    return 1/spectrum


# Inicializo la clase ILT
ilt = ILT(rango=(1e-1, 1e4), kernel=KERNEL, Nilt=Nilt,
            figure=2, savepath=None, alpha=alpha)
# calculo la ILT para el conjunto de datos 1
ilt.DoTheStuff(1/simulated_experiment, simulated_ppm, muestra=fr"$\alpha$={ilt.alpha:.0e}")
ilt.plot_fit()
...
for k in np.logspace(-1, 3, Nilt):
    suma += KERNEL(simulated_ppm, k)
plt.plot(simulated_ppm, suma, label=f'k_ex={k:.0e} Hz', alpha=0.5)
