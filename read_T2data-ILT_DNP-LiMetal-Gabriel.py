...
datos = DatosProcesadosT1(path, vdlist_path="lists/vd/sm2974_T2")
...
for ii in range(Nsegments):
    ...
    ilt = ILT(alpha=alpha, rango=(3e-5, 1e-2), kernel=T2, Nilt=Nilt, figure=ii, savepath=savepath)
...
ax.pcolormesh(ppmAxis_reduced, ilt.xilt, iltft, vmax=0.5*iltft.max())
ax.set_yscale("log")
ax.set_xlim(np.max(ppmAxis_reduced), np.min(ppmAxis_reduced))
