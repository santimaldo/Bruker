...
savepath = "S:/tmp/"
mT = [1,100, 350, 1000, 10, 35, 600, 5, 20, 200, 75, 275, 50, 800, 150, 700, 900, 500, 420, 120, 1200, 1500] #1H
...
size = 10
figura = plt.figure(1, figsize=(size, size*filas/columnas),constrained_layout=False)
...
for n in range(len(nexp)):
    ...
    espectros1d.append(Espectro1D.real) ### espectro 1D como el primer espectro de la exsy
...
for n in range(len(espectros1d)):
  ...
  plt.plot(ppm_x, np.abs(spec1d)/np.max(np.abs(spec1d)), label=f"mixing time: {mT[n]:.0f} ms")
plt.legend()
