...
for kk in range(2*last_echo_number):
    ...
    Signals_err = np.append(Signals_stderr, 1.96*signal_stderr)
...
for internuc_distance in np.arange(0.4, 0.61, 0.05):
    ...
    NTr, S_S0 = redor_curve(internuc_distance, spin_speed, Nsim)
    xvals = NTr * 1000
...
ax_redor.errorbar(recopl_time[:Necos]*1000, S[:Necos]/S0[:Necos],
                  yerr=Signals_err[::2][:Necos]/S0[:Necos],
...
fig_t2, ax_t2 = plt.subplots()
ax_t2.plot(recopl_time[:Necos]*1000, S[:Necos],'o-', label='S')
ax_t2.plot(recopl_time[:Necos]*1000, S0[:Necos], 'o-', label='S0')
