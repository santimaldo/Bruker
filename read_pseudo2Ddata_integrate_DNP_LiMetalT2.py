...
expns, sample = [5, 9 , 29], 'Li on Cu mesh'
...
plotRange = [500,150]
ppmIntegrationWidth = 100  # ancho de la ventana de integracion
...
path = rf"C:\Users\Santi\OneDrive - University of Cambridge\NMRdata\400dnp\2025-06-27_InSitu/"
...
for jj, expn in enumerate(expns):
    ...
    re = datos.espectro.real[0]
    im = datos.espectro.imag[0]
...
plt.title(sample)
plt.bar(np.arange(T1_list.size), T1_list, label='Experimental $T_1$')
plt.xlabel("nexp")
plt.ylabel(r"$T_2$ [ms]")
plt.grid(axis='y')
plt.tight_layout()
plt.xticks(np.arange(T1_list.size), xlabels)#, rotation=45)
