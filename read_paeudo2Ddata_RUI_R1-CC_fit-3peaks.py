...
Ahora con ajuste de 3 picos + 1 pico (m0_), y color gris para el ajuste de 1 pico
@author: Santi
"""
...
m1_center, m2_center, m3_center = [240.2, 247, 251.8]
m1_sigma,  m2_sigma,  m3_sigma  = m0_sigma, m0_sigma, m0_sigma
...
vfit_results = pd.DataFrame(columns=[
    ...
    'm3_fwhm',
    'r-suqared_1peak', 'r-squared_3peaks'
])
...
for jj, expn in enumerate(expns):
    ...
    vfit = PseudoVoigtFit(
        ...
        #fijar = ["sigma"],
        bounds={"m1_center": [m1_center-2.5, m1_center+2.5],
...
ax_int.plot(time, m3_amplitude/signals.max(), 'go', label='peak 3')
ax_int.plot(time, signals/signals.max(), 'ko', label='total signal: peaks 1+2+3')
...
ax.plot(time, m3_center, 'go', label='peak 3')
